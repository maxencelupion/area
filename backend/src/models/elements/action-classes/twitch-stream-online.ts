...
@Injectable()
export class TwitchActionNewStreamOnline extends BaseAction {
  constructor(
    serviceId: number,
    ...
    super(
      ...
      'a followed streamer starts a live',
      'action',
      serviceId,
  ...
  private async registerStreamers(userId: number, access_token: string) {
    ...
    for (const elem of allFollows) {
      ...
      if (status && !existingFollow.trigger_on) {
        await this.followService.changeTriggerOn(existingFollow.id, true);
        return existStreamer;
      }
      if (!status && existingFollow.trigger_on) {
        await this.followService.changeTriggerOn(existingFollow.id, false);
      }
    }
  ...
  async execAction(userId: number, parameters: any) {
    ...
    if (res) {
      return {
        ...
        title: `${res.streamerName} est en live`,
        body: `Rejoignez le live de ${res.streamerName} dès maintenant !`,
      };
    }
  }
